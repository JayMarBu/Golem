...
namespace golem {
	...
	void SwapChain::CreateImageViews()
	{
		...
		for (size_t i = 0; i < m_swapChainImages.size(); i++)
		{
			...
			SAFE_RUN_VULKAN_FUNC(vkCreateImageView(m_device.device(), &viewInfo, nullptr, &m_swapChainImageViews[i]),
	...
	void SwapChain::CreateRenderPass()
	{
		...
		SAFE_RUN_VULKAN_FUNC(vkCreateRenderPass(m_device.device(), &renderPassInfo, nullptr, &m_renderPass), "failed to create render pass!");
	}
	...
	void SwapChain::CreateFramebuffers()
	{
		...
		for (size_t i = 0; i < imageCount(); i++)
		{
			...
			SAFE_RUN_VULKAN_FUNC(vkCreateFramebuffer(m_device.device(), &framebufferInfo, nullptr, &m_swapChainFramebuffers[i]),
	...
	void SwapChain::CreateDepthResources()
	{
		...
		for (int i = 0; i < m_depthImages.size(); i++)
		{
			...
			SAFE_RUN_VULKAN_FUNC(vkCreateImageView(m_device.device(), &viewInfo, nullptr, &m_depthImageViews[i]),
				"failed to create texture image view!");
		}
	}
	...
	VkPresentModeKHR SwapChain::ChooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes)
	{
		for (const auto& availablePresentMode : availablePresentModes)
		{
			...
			{
				GOL_CORE_INFO("Present mode: Mailbox");
		...
		//     std::<< "Present mode: Immediate" << std::endl;
		//     return availablePresentMode;
		//   }
		// }

		GOL_CORE_INFO("Present mode: V-Sync");
